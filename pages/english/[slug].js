...
const slug = ({ EnglishSubCaption }) => {
  // eslint-disable-next-line react-hooks/rules-of-hooks
  const router = useRouter();
  const { slug } = router.query;
  // eslint-disable-next-line react-hooks/rules-of-hooks
  const [captionData, setCaptionData] = useState(EnglishSubCaption);
  ...
  return (
    <>
      ...
      <div>
        <h1 className="text-fuchsia-600 text-center lg:text-4xl my-10">
          English {slug} captiondata
        </h1>
        <button
          onClick={back}
